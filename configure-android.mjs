...
for (const permission of unwanted) {
  ...
  const pattern = new RegExp(`\\s*<uses-permission[^>]+android:name=["']${escaped}["'][^>]*/>\\s*`, 'g');
  xml = xml.replace(pattern, '\n');
}
fs.writeFileSync(manifest, xml);

const remaining = unwanted.filter(p => xml.includes(`android:name="${p}"`) || xml.includes(`android:name='${p}'`));
